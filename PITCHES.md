...
**Product one-liner:** WARD is rails for an autonomous system to hire and pay a verified human for physically-verifiable work, settled on-chain. The instrumented home is the first instance, not the product: the home agent watches your devices, fixes what it can in software, and when the fault is physical it hires and pays a verified human on attested completion. Crypto is load-bearing only when it pays a human, which is why the real buyers are software with no bank account (property managers, DePIN fleets, DAO treasuries, agent wallets).

**The escalation ladder (lead with this when a judge thinks "it just hires people").** WARD does not jump to hiring a human. It climbs a ladder, cheapest first. **L1 self-fix** is free, instant, and autonomous: software remedies the agent runs itself (reboot, reconfigure, re-pair, cycle a relay, close a smart valve). Most incidents end here, and this is the everyday value. **L2 guided remote** is an optional scripted multi-step remediation, still no human. **L3 hire a human** is the last resort, used only when the fault is physical and software cannot resolve it: it spends money and dispatches a person, so it runs only within the owner's spending policy (per-job cap, daily cap, owner-approval threshold). In the demo a WiFi fault is fixed at L1 by a remote reboot, no human and no escrow; the 2am leak fails L1 because the burst is upstream of the valve, so it escalates to L3 and hires a plumber. That contrast is the point: the agent is intelligent, not "always hires a human."

**The leak demo:** You're in Tokyo, it's 2am back home. The leak sensor in your Brooklyn apartment triggers. WARD tries the free L1 fix and water is the one fault software almost never resolves, so it escalates to L3: it discovers and ranks the registered workers via ENS, hires Mike (`mike.ward-agent.eth`), escrows USDC as an ERC-8183 Job on Arc, and dispatches him. Mike fixes the leak, the moisture sensor reads dry, a Chainlink CRE workflow attests it on-chain, and the USDC releases. You slept through the whole thing. (The cinematic narrates a 150 USDC dispatch fee for legibility; the real on-chain settled jobs are 1 USDC, faucet-bounded but real.)
...
**Q5. "Isn't this just Alexa / a cloud home assistant?"**
No, and the gap is the whole point. A cloud assistant can observe and notify: it can tell you the leak sensor tripped, maybe toggle a smart plug. What it cannot do is settle with a human in the physical world. That is WARD's primitive: when software cannot fix the fault, the agent hires a verified worker, escrows USDC, and releases it on a machine-attested fact instead of a human tap, all under the owner's spending policy. A cloud assistant has no escrow, no portable worker identity, and no way to pay a person on proof. WARD self-fixes in software when it can (the L1 ladder, where a cloud assistant tops out) and dispatches and pays a human when it can't (L3, which no cloud assistant can do). What is on-chain is the settlement, the identity, and the reputation: ENS for portable worker identity, Arc for the escrow. That is exactly the part that generalizes past the home, to any software buyer that has to pay humans for verifiable work and cannot use a bank.
